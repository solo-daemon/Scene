...
export default function SendNotification() {
  ...
  const subscribeButtonOnClick: MouseEventHandler<HTMLButtonElement> = async (event) => {
    ...
    await apiPost.post("/save-subscription/", {
    ...
    <>
      ...
      <PopoverTrigger asChild>
        <button type="button" onClick={subscribeButtonOnClick} disabled={isSubscribed}>
          {isSubscribed ? (
            <Bell color="#4E55E1" className="w-4 h-4" />
          ) : (
            <Bell color="#000000" className="w-6 h-6" />
          )}
        </button>
      </PopoverTrigger>
      <PopoverContent className="w-64 text-sm">
        🎉 Hey! Want to get fun updates or dares from your friends?<br />
        Click the bell to subscribe to notifications!
      </PopoverContent>
    </Popover>
    <button onClick={unsubscribeButtonOnClick}>unsubcribe</button>
    </>
  );
}